...
const getToolsByCategory = (categoryName) => {
  return tools.filter(tool => {
    ...
    // 获取当前分类的子分类选择
    const currentSubCategory = categorySubCategories.value[categoryName]
    
    // 如果选择的是热门推荐，返回该分类下的热门工具
    if (currentSubCategory === '热门推荐') {
    ...
    <nav class="sidebar">
      ...
      <div class="nav-links">
        ...
        >
          ...
          >
            <a 
              v-for="item in getSubmenu(category.name)"
              :key="item"
              :href="'#' + category.name"
              class="submenu-item"
        ...
        <div class="content-header">
          <div class="header-wrapper">
            ...
            <div class="category-tags" v-if="category.hasSubmenu">
              <a href="#" 
                v-for="tag in getSubmenu(category.name)" 
                :key="tag"
